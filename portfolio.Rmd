...
먼저 샘플로 사용할 주가 및 재무제표 데이터를 다운로드 받습니다. 데이터의 아래 링크에 .rds 형태로 업로드되어 있습니다.

<https://github.com/hyunyulhenry/dmmr_quant/blob/main/data.rds>

코드를 이용해 바로 다운로드 받도록 하겠습니다.

```{r eval = FALSE}
download.file('https://github.com/hyunyulhenry/dmmr_quant/raw/main/data.rds', 'data.rds', mode = 'wb')
```
...
리스트 내에 티커, 섹터, 가격, 재무제표, 가치지표 정보가 들어가 있습니다.
...
```{r}
ticker = data$kor_ticker
value = data$kor_value
```

먼저 티커 테이블과 가치지표 테이블을 불러옵니다.
...
1.  티커 테이블에서 가장 최근일 데이터를 선택한 후, 종목코드와 종목명을 선택합니다.
2.  밸류 테이블에서 가장 최근일 데이터를 선택합니다.
3.  일부 종목은 가치지표가 0보다 작은 경우(예: 적자기업의 경우 PER가 음수, 혹은 배당수익률이 0%인 종목)가 있으며 이러한 데이터는 NA로 변경합니다.
4.  테이블을 가로로 긴 형태로 변경합니다.
5.  두 테이블을 합칩니다.
...
1.  먼저 티커 테이블과 가격 테이블을 불러옵니다.
2.  가격 테이블에서 종목코드 별로 그룹을 묶은 다음 최근 1년에 해당하는 데이터만 선택합니다.
...
1.  종목코드 별로 그룹을 나눈 후 최근 1년치 데이터를 선택합니다.
2.  최근 1년 데이터가 없는 경우, 즉 상장한지 1년이 되지 않는 기업은 제외합니다.
3.  수익률을 계산합니다.
4.  `do()` 함수를 통해 한 번에 K-Ratio를 구합니다. 이는 `for loop` 구문을 이용해 계산하는 것보다 훨씬 빠릅니다.
...
fs = data$kor_fs %>%
  filter(계정 %in% c('당기순이익', '매출총이익', '영업활동으로인한현금흐름', '자산', '자본'),
         공시구분 == 'q')
...
하나의 팩터만을 보고 투자하는 것보다, 둘 혹은 그 이상의 팩터를 결합해 투자해야 훨씬 좋은 포트폴리오를 구성할 수 있으며, 이러한 방법을 멀티팩터라고 합니다. 그중에서도 밸류와 퀄리티의 조합은 전통적으로 많이 사용된 방법이며, 대표적인 예가 조엘 그린블라트의 ‘마법공식’입니다. 이에 앞서, 퀄리티와 밸류 간의 관계, 마법공식의 정의와 구성 방법을 알아보겠습니다.

## 퀄리티와 밸류 간의 관계

투자의 정석 중 하나는 좋은 기업을 싸게 사는 것입니다. 이를 팩터의 관점에서 이해하면 퀄리티 팩터와 밸류 팩터로 이해할 수도 있습니다.

여러 논문에 따르면 흔히 밸류 팩터와 퀄리티 팩터는 반대의 관계에 있습니다. 먼저 가치주들은 위험이 크기 때문에 시장에서 소외를 받아 저평가가 이루어지는 것이며, 이러한 위험에 대한 대가로 밸류 팩터의 수익률이 높습니다. 반대로 사람들은 우량주에 기꺼이 프리미엄을 지불하려 하기 때문에 퀄리티 팩터의 수익률이 높기도 합니다. 이는 마치 동전의 양면과 같지만, 장기적으로 가치주와 우량주 모두 우수한 성과를 기록합니다. 먼저 퀄리티 지표인 매출총이익과 밸류 지표인 PBR을 통해 둘 사이의 관계를 확인해 봅시다.
...
fs = data$kor_fs %>%
  filter(계정 %in% c('매출총이익', '자산'), 공시구분 == 'y') %>%
  group_by(종목코드, 계정) %>%
  slice(n()) %>%
  ungroup() %>%
  pivot_wider(names_from = 계정, values_from = 값) %>%
  mutate(GPA = 매출총이익 / 자산)

bind_rank = value %>% select(종목코드, PBR) %>%
  left_join(fs %>% select(종목코드, GPA)) %>%
  mutate(rank_PBR = rank(PBR),
         rank_GPA = rank(desc(GPA)))

bind_rank %>% select(contains('rank')) %>%
  cor(use = 'complete.obs')
```

1. 밸류 지표와 재무제표 테이블을 가져옵니다.
2. 연간 기준 GPA를 계산합니다.
3. PBR과 GPA의 랭크를 구합니다.
4. 순위의 상관관계를 구합니다.

PBR과 GPA 간에는 음의 상관관계가 있음이 확인됩니다. 이번에는 PBR의 분위수별 GPA 평균값을 구해보도록 합시다.

```{r}
bind_rank %>%
  na.omit() %>%
  mutate(PBR_quantile = ntile(PBR, 5)) %>%
  group_by(PBR_quantile) %>%
  summarize(mean_GPA = mean(GPA)) %>%
  ggplot(aes(x = PBR_quantile, y = mean_GPA)) +
  geom_col()
```

1. `na.omit()` 함수를 통해 NA가 있는 데이터를 제거합니다.
2. `ntile()` 함수를 통해 PBR 기준 5분위로 나눕니다.
3. 분위별로 그룹을 묶은 후, GPA의 평균을 구합니다.

그림에서 알 수 있듯이 PBR이 낮을수록 GPA도 낮으며, 즉 가치주일수록 우량성은 떨어집니다다. 반면에 PBR이 높을수록 GPA도 높으며, 이는 주식의 가격이 비쌀수록 우량성도 높다는 뜻입니다. 이를 이용해 밸류 팩터와 퀄리티 팩터 간의 관계를 나타내면 다음과 같습니다.

![](images/portfolio/tableqv.png)

주가가 쌀수록(저밸류) 기업의 우량성은 떨어지며(①번), 반대로 기업의 우량성이 좋으면 주식은 비싼 경향(③번)이 있습니다. 물론 우량성도 떨어지고 비싸기만한 주식(②번)을 사려는 사람은 아마 없을 것입니다. 결과적으로 우리가 원하는 최고의 주식은 우량성이 있으면서도 가격은 싼 주식(④번)입니다.

## 마법공식 포트폴리오

마법공식이란 고담 캐피탈의 설립자이자 전설적인 투자자 조엘 그린블라트에 의해 알려진 투자 방법입니다. 그는 본인의 책 《주식 시장을 이기는 작은 책》에서 투자를 하는데 있어 중요한 두 가지 지표가 있으며, 이를 혼합하면 뛰어난 성과를 기록할 수 있다고 말했습니다.

첫 번째 지표는 ‘이익 수익률(Earnings Yield)’ 로써 기업의 수익을 기업의 가치로 나는 값입니다. 이는 PER의 역수와 비슷하며, 밸류 지표 중 하나입니다. 두 번째 지표는 ‘투하자본 수익률(Return on Capital)’ 로써 기업의 수익을 투자한 자본으로 나눈 값입니다. 이는 ROE와도 비슷하며, 퀄리티 지표 중 하나입니다. 마법공식은 이 두 가지 지표의 순위를 각각 구한 후 순위의 합 기준 상위 30~50개 종목을 1년간 보유한 후 매도하는 전략입니다.

재무제표 항목을 통해 ROE와 PER를 계산하고, 이를 통해 마법공식 포트폴리오를 구성해보도록 합시다.
...
fs = data$kor_fs %>%
  filter(계정 %in% c('당기순이익', '자본'), 공시구분 == 'q') %>%
  arrange(종목코드, 계정, 기준일) %>%
  group_by(종목코드, 계정) %>%
  mutate(rollsum = roll_sum(값, n = 4, align = 'right', fill = NA)) %>%
  slice(n()) %>%
  mutate(rollsum = case_when(
    계정 %in% c('자본') ~ rollsum / 4,
    TRUE ~ rollsum
  )) %>%
  ungroup() %>%
  select(종목코드, 계정, rollsum) %>%
  pivot_wider(names_from = 계정, values_from = rollsum) %>%
  mutate(ROE = 당기순이익 / 자본)
```

먼저 티커, 가치지표, 재무제표 테이블을 불러온 후 ROE를 계산합니다.

```{r}
magic = ticker %>%
  left_join(value %>% select(종목코드, PER)) %>%
  left_join(fs %>% select(종목코드, ROE)) %>%
  mutate(PER_rank = min_rank(PER),
         ROE_rank = min_rank(desc(ROE))) %>%
  mutate(rank = min_rank(PER_rank + ROE_rank))

magic %>% filter(rank <= 20)
```

테이블 중 필요한 열만 선택하여 결합합니다. 그 후 PER와 ROE의 랭킹을 통해 상위 20 종목을 선택합니다. 선택된 종목들을 보면 ROE가 높으면서도 PER가 낮은 종목들로 구성되어 있습니다. 이번에는 전체 종목과 선택된 종목들의 차이를 시각화 해보도록 하겠습니다.

```{r}
magic %>%
  mutate(invest = ifelse(rank <= 20, 'Y', 'N')) %>%
  na.omit() %>%
  ggplot(aes(x = PER, y = ROE, color = invest, shape = invest)) + 
  geom_point(size = 5, alpha = 0.5) +
  xlim(0, 10) +
  ylim(0, 1.5)
```

전체 종목 중 마법공식에 해당하는 종목들이 좌측 상단, 즉 ROE가 높고 PER는 낮은 지점에 위치하고 있습니다.
...
먼저 티커, 섹터, 주가, 재무제표, 가치지표 데이터를 불러옵니다. 
...
먼저 퀄리티 지표를 계산하기 위해 TTM 기준 ROE, GPA, CFO를 계산합니다.
...
data_bind = data_bind %>%
  mutate(SEC_NM_KOR = replace_na(SEC_NM_KOR, '기타')) %>% as.tibble()
