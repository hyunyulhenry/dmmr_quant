...
셀레니움은 다양한 브라우저(인터넷 익스플로러, 크롬, 사파리 오페라 등) 및 플랫폼에서 웹 응용 프로그램을 테스트할 수 있게 해주는 라이브러리 입니다. 즉 웹 자동화 테스트 용도로 개발이 되었기에 실제 브라우저를 사용하며, 페이지가 변화하는 것도 관찰이 가능하기에 동적 크롤링에 사용할 수 있습니다.

셀레니움을 사용하기 전에 어떠한 브라우저를 사용할 지 정해야하며, 일반적으로 크롬을 사용합니다. 먼저 본인의 크롬 브라우저 버전을 확인합니다.

![](images/api/chrome.png)

1. 크롬 브라우저를 연 후 우측 상단의 점세개 버튼을 클릭합니다.
2. 설정을 클릭합니다.
3. 좌측 맨 하단의 Chrome 정보를 클릭합니다.
4. 본인의 버전을 확인합니다.

본인의 버전에 맞는 크롬 드라이버는 아래 사이트에서 다운로드가 가능합니다. 만일 확인한 버전이 106.0.xxx 일 경우는 If you are using Chrome version 106… 뒤의 링크를 클릭하면 됩니다. 그 후 linux/mac/win 중 본인의 운영체제에 맞는 파일을 다운로드 한 후 압축을 풉니다. (윈도우 64비트 사용자의 경우 32비트를 받아도 무관합니다.)

https://chromedriver.chromium.org/downloads

![](images/api/chrome_down.png)

![](images/api/chrome_down_2.png)

향후 크롬 브라우저가 업데이트 될 경우, 크롬 드라이버 역시 업데이트된 버전으로 다시 다운로드 받아야 합니다. 다음으로 geckodriver를 다운로드 받아야 합니다. 아래 사이트에서 본인의 운영체제에 해당하는 파일을 다운로드 받은 후 압축을 풉니다.

https://github.com/mozilla/geckodriver/releases/tag/v0.17.0

![](images/api/gecko.png)
 
마지막으로 아래 사이트에서 셀레니움을 [selenium-server-standalone-4.0.0-alpha-1.jar]를 다운받습니다.

http://selenium-release.storage.googleapis.com/index.html?path=4.0/

![](images/api/sel.png)

크롬 브라우저와 geckodrivers는 zip 파일 압축을 풉니다. 이 후 cmd를 입력해 명령 프롬프트 창으로 진입하여 다음과 같이 입력합니다. (혹은 Console 우측의 Terminal을 이용해도 됩니다.)
...
셀레니움 파일경로에는 파일들이 있는 경로를, selenium-server-4.0.0.jar에는 셀레니움 파일 명을 입력합니다. (버전마다 다를 수 있습니다.) 제대로 입력이 되면 아래와 같은 화면이 뜨며 4445번 포트에 셀레니움이 구동됩니다.

![](images/api/install.png)

**해당 CMD 창은 계속 켜두고, R로 돌아옵니다.** 그 후 아래의 코드를 입력합니다. 
...
위 코드를 치면 크롬 창이 열리며, 좌측 상단에 [Chrome이 자동화된 테스트 소프트웨어에 의해 제어되고 있습니다.]라는 문구가 뜹니다. 이제 코드를 이용해 해당 페이지를 조작할 수 있습니다.

![](images/api/sel_1.png)

```{r eval = FALSE}
remD$navigate("https://www.naver.com/")
unlist(remD$getPageSource())
```

`remD$navigate()` 내에 URL 주소를 입력하면 해당 주소로 이동합니다. 또한 `remD$getPageSource()`를 통해 열려있는 창의 HTML 코드를 확인할 수도 있습니다.

이제 네이버 메인에서 [뉴스]버튼을 누르는 동작을 실행해봅시다. 개발자도구 화면을 통해 확인해보면 [뉴스] 탭은 아래 HTML에 위치하고 있습니다.

![](images/api/sel_2.png)
```{r eval = FALSE}
remD$findElement(using = 'xpath', '//*[@id="NM_FAVORITE"]/div[1]/ul[2]/li[2]/a')$clickElement()
```

브라우저 상에서 보이는 버튼, 검색창, 사진, 테이블, 동영상 등을 엘레먼트(element, 요소)라고 합니다. `findElement()`는 다양한 방법으로 엘레먼트에 접근하게 해주며, using을 통해 어떠한 방법으로 엘레먼트에 접근할지 선언합니다. 여기서는 xpath를 통해 해당 부분에 접근하였습니다. `clickElement()`는 마우스 클릭을 실행하며 결과적으로 뉴스 탭을 클릭한 후 페이지가 이동되는 것을 확인할 수 있습니다.

![](images/api/sel_3.png)
...
`goBack()`은 뒤로가기를 의미하며, 기존 페이지던 네이버 메인으로 이동합니다.

이제 특정 검색어를 검색하는 방법에 대해 알아보겠습니다. 먼저 검색창의 위치가 어디에 있는지 확인해보면 query라는 id와 input_text라는 class에 위치하고 있습니다.

![](images/api/sel_4.png)

```{r eval = FALSE}
remD$findElement(using = 'class', 'input_text')$sendKeysToElement(list("퀀트 투자 포트폴리오 만들기"))

```

findElement 내에 **class를** 입력하면 클래스 명이 있는 엘레먼트에 접근하며, 여기서는 검색창에 접근합니다. 그 후 `sendKeysToElements()` 내에 리스트 형태로 텍스트를 입력하면 해당 내용이 웹페이지에 입력됩니다.

이제 웹페이지에서 검색 버튼 해당하는 돋보기 모양을 클릭하거나 엔터키를 누르면 검색이 실행됩니다. 먼저 돋보기 모양의 위치를 확인해보면 **search_btn** id와 **btn_submit** 클래스에 위치하고 있습니다.

![](images/api/sel_5.png)

```{r eval = FALSE}
remD$findElement(using = 'class', 'btn_submit')$clickElement()
```

![](images/api/sel_6.png)

`findElement(using = 'class', 'btn_submit')`를 통해 검색 버튼에 접속합니다. 그 후 `clickElement()` 를 통해 클릭 동작을 수행합니다다. 페이지를 확인해보면 검색이 실행된 후 결과를 확인할 수 있습니다.

이번에는 다른 단어를 검색해보도록 합시다. **웹에서 기존 검색어 내용을 지운 후, 검색어를 입력하고, 버튼을 클릭**해야 합니다. 이를 위해 검색어 박스의 위치를 찾아보면 **box_window** 클래스에 위치하고 있습니다.
...
![](images/api/sel_7.png)

1. 검색어 박스에 접근한 후, `clearElemen()`를 실행하면 모든 텍스트가 지워집니다.
2. `sendKeysToElement('이현열 퀀트')`를 실행하여 새로운 검색어를 입력한 후, `key = "enter"`를 추가하면 엔터 동작이 실행됩니다.

이번에는 [VIEW] 버튼을 클릭하는 동작을 실행해보도록 합니다. 해당 부분의  XPATH 위치는 다음과 같습니다.

```
//*[@id="lnb"]/div[1]/div/ul/li[2]/a
```

이를 이용해 해당 부분을 클릭하는 동작을 실행합니다.

```{r eval = FALSE}
remD$findElement(using = 'xpath', '//*[@id="lnb"]/div[1]/div/ul/li[2]/a')$clickElement()
```

![](images/api/sel_8.png)

[통합] 검색이 아닌 [VIEW]으로 변경되었습니다. 이번에는 [옵션]을 클릭한 후 정렬을 [최신순]으로 하는 동작을 실행해봅니다. 둘의 위치는 다음과 같습니다.

- [옵션] 버튼: `option_filter` 클래스
- 최신순 버튼: `//*[@id="snb"]/div[2]/ul/li[2]/div/div/a[2]` XPATH
 
먼저 아래의 [X] 버튼을 눌러 해당 창을 닫습니다.

![](images/api/sel_9.png)

그 후 아래의 코드를 입력해 [옵션 클릭 → 최신순 선택]의 동작을 수행합니다.

```{r eval = FALSE}
remD$findElement(using = 'class', 'option_filter')$clickElement()
remD$findElement(using = 'xpath', '//*[@id="snb"]/div[2]/ul/li[2]/div/div/a[2]')$clickElement()
```

옵션 클릭 후 최신순 버튼을 클릭하는 동작을 수행하여 검색어가 최신순으로 정렬되었습니다. 이제 page down 기능을 수행해보도록 합시다.

```{r eval = FALSE}
remD$executeScript("window.scrollTo(0,document.body.scrollHeight);")
```

먼저 `document.body.scrollHeight`는 웹페이지의 높이를 나타내며, `window.scrollTo(0, document.body.scrollHeight);`는 웹페이지의 가장 하단까지 스크롤을 하라는 자바스크립트 명령어입니다.

그러나 결과를 살펴보면 스크롤이 끝까지 내려가며 얼마간의 로딩이 있은 후 새로운 데이터가 생성됩니다. 이처럼 유튜브나 인스타그램, 페이스북 등 많은 검색결과를 보여줘야 하는 경우 웹페이지 상에서 한 번에 모든 데이터를 보여주기 보다는 스크롤을 가장 아래롤 위치하면 로딩을 거쳐 추가적인 결과를 보여줍니다. 따라서 스크롤을 한 번만 내리는 것이 아닌 모든 결과가 나올 때까지 내리는 동작을 실행할 필요가 있습니다.

```{r eval = FALSE}
prev_height = remD$executeScript('return document.body.scrollHeight')

while (TRUE) {
  remD$executeScript('window.scrollTo(0, document.body.scrollHeight);')
  Sys.sleep(2)
  
  curr_height = remD$executeScript('return document.body.scrollHeight')
...
1. return document.body.scrollHeight은 현재의 창 높이는 반환하는 자바스크립트 명령어이며, 이를 prev_height에 저장합니다.
2. while 구문을 통해 반복문을 실행합니다.
3. 셀레니움을 통해 페이지의 최하단으로 스크롤을 내립니다.
4. 페이지가 로딩되는 시간을 기다리기 위해 2초간 슬립을 줍니다.
5. curr_height에 현재 창 높이를 저장합니다.
6. curr_height와 prev_height가 동일하다는 의미는 페이지가 끝까지 내려왔다는 의미입니다. 따라서 이 경우 break를 통해 while 구문을 멈추며, 그렇지 않을 경우 다시 스크롤을 내리는 동작을 반복합니다.
7. prev_height에 새로운 창 높이를 입력한다.

이제 모든 검색 결과가 나타났으면 이전 장에서 살펴보았던 정적 크롤링을 통해 데이터 수집이 가능합니다. 제목 부분을 확인해보면 `api_txt_lines total_tit _cross_trigger` 클래스에 위치하고 있으며, 이를 통해 모든 제목을 크롤링해봅시다.

```{r eval = FALSE}
html = read_html(unlist(remD$getPageSource()))
txt_list = html %>%
  html_nodes('.api_txt_lines.total_tit._cross_trigger') %>%
  html_text()

txt_list %>% head()
```

이처럼 동적 페이지의 경우도 셀레니움을 통해 웹페이지를 제어한 후 원하는 부분을 추출하면 얼마든지 크롤링이 가능합니다.

```{r eval = FALSE}
remD$quit()
```

마지막으로 열려있던 페이지를 종료합니다.

RSelenium에 대한 더욱 자세한 내용은 아래 페이지에 나와있습니다.
